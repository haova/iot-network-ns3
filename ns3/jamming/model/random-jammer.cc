...
RandomJammer::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::RandomJammer")
    ...
    .AddAttribute ("RandomJammerRandomInterval",
                   ...
                   MakePointerAccessor (&RandomJammer::SetRandomVariable),
    ...
    .AddAttribute ("RandomJammerReactToMitigationFlag",
                   ...
                   UintegerValue (false), // default to not chasing
...
RandomJammer::RandomJammer ()
  :  m_reactToMitigation (false),
     m_reacting (false)
{
  m_randomJammingInterval = CreateObject<UniformRandomVariable> ();
}
...
void
RandomJammer::SetRandomVariable (Ptr<UniformRandomVariable> random)
{
  NS_LOG_FUNCTION (this);
...
RandomJammer::DoEndTxHandler (Ptr<Packet> packet, double txPower)
{
 ...
 /*double f = (double)rand() / 1 ;
  NS_LOG_INFO(f);
    f=  0.001 + f * (0.005 - 0.001);*/

   double f = (0.01-0.001) * ((double) rand() / (double) RAND_MAX);

   NS_LOG_INFO(f);
    NS_LOG_INFO(m_jammingDuration.GetSeconds());
    
  Time intervalToNextJamming = Seconds (f);
   NS_LOG_INFO(intervalToNextJamming);
