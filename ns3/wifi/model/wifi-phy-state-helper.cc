...
namespace ns3
{
  ...
  {
    NS_LOG_FUNCTION(this << packet << snr << txVector << statusPerMpdu.size() << std::all_of(statusPerMpdu.begin(), statusPerMpdu.end(), [](bool v)
                                                                                             { return v; })); // returns true if all true
