...
public:
  ...
  typedef struct PhyLayerInfo
  {
    ...
    std::vector <uint16_t> tableChannel;
  } PhyLayerInfo;
...
public:
  ...
  uint32_t GetNumberJamming();

  uint16_t GetTotalRetransmission();
...
private:
  ...
  std::vector<uint16_t> m_packetSequence;
