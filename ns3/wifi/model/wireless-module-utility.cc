...
namespace ns3
{
  ...
  WirelessModuleUtility::GetTypeId(void)
  {
    static TypeId tid = TypeId("ns3::WirelessModuleUtility")
                            ...
                            .AddAttribute("PdrWindowSize",
                                          ...
                                          UintegerValue(8600), // default to 40 packets
                            ...
                            .AddAttribute("RssUpdateInterval",
                                          ...
                                          TimeValue(Seconds(0.1)), // default to 0.5 second
  ...
  uint16_t
  WirelessModuleUtility::GetTotalRetransmission()
  {
                                      ...
                                      const bool isSuccessfullyReceived)
  {
    ...
    uint16_t sequence = head.GetSequenceNumber();
    NS_LOG_DEBUG(sequence);
    uint16_t flag = 0;
    uint16_t len = m_packetSequence.size();
    NS_LOG_DEBUG(len);

    if (len == 0)
    {
      m_packetSequence.push_back(sequence);
    }
    else
    {

      for (const auto &item : m_packetSequence)
      {
        if (item == sequence)
        {
          flag = 1;
  ...
  {
    NS_LOG_FUNCTION(this << m_nodeRssW);
    return m_nodeRssW;
  }

  double
  WirelessModuleUtility::GetRssinDbm(void) const
  {
    NS_LOG_FUNCTION(this << m_rssinDbm);
    return m_rssinDbm;
  }
    ...
    while (pktItemIterator.HasNext())
    {
      ...
      if (item.isFragment)
      {
        ...
        case PacketMetadata::Item::TRAILER:
          ...
          {
            NS_LOG_DEBUG("WirelessModuleUtility:" << itemName << " Found!"
                                                  << " Size = " << size << " bytes");
            return size;
          }
          break;
        }
        NS_LOG_DEBUG("WirelessModuleUtility:"
                     << " Fragment [" << item.currentTrimmedFromStart << ":" << (item.currentTrimmedFromStart + item.currentSize) << "]");
      } // end if
      else
      {
        switch (item.type)
  ...
  WirelessModuleUtility::UpdatePdr(const bool isPacketValid)
  {
    ...
    NS_LOG_DEBUG("WirelessModuleUtility:Updated PDR = " << m_Pdr << ", Total received packets = " << totalPkts << ", # of valid packets = " << validPkts);
  ...
  uint32_t
  WirelessModuleUtility::GetNumberJamming()
  {

    uint32_t numberJamming = m_totalPkts - m_validPkts;

    NS_LOG_FUNCTION(this << numberJamming);
    return numberJamming;
  }
  ...
  WirelessModuleUtility::UpdateRss(void)
  {
    ...
    NS_LOG_DEBUG("WirelessModuleUtility:At time = " << Simulator::Now().GetSeconds() << " s"
                                                    << ", Current RSS = " << m_nodeRssW << " W"
                                                    << ", in dBm " << WToDbm(m_nodeRssW));

    m_rssinDbm = WToDbm(m_nodeRssW);
  ...
  WirelessModuleUtility::SendJammingSignal(double power, Time duration)
  {
    ...
    {
      NS_LOG_ERROR("WirelessModuleUtility:Power of " << power << " W is not supported by PHY layer, max power = " << m_phyLayerInfo.maxTxPowerW << ", scaling to max power!");
      power = m_phyLayerInfo.maxTxPowerW;
    }
    else if (power < m_phyLayerInfo.minTxPowerW)
    {
      NS_LOG_ERROR("WirelessModuleUtility:Power of " << power << " W is not supported by PHY layer, min power = " << m_phyLayerInfo.minTxPowerW << ", scaling to min power!");
