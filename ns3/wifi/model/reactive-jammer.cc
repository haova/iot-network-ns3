...
namespace ns3
{
  ...
  {
    NS_LOG_FUNCTION(this);
    NS_LOG_DEBUG("ReactiveJammer:At Node #" << GetId() << ", Started!");

    if (m_reactToMitigation)
    {
      NS_LOG_DEBUG("ReactiveJammer:At Node #" << GetId() << ", After jammer starts, scheduling RX timeout!");
  ...
  ReactiveJammer::DoStartRxHandler(Ptr<Packet> packet, double startRss)
  {
    ...
    NS_LOG_DEBUG("ReactiveJammer:At Node #" << GetId() << ", Started receiving a packet!");

    if (IsPacketToBeJammed(packet))
    {
      NS_LOG_DEBUG("ReactiveJammer:At Node #" << GetId() << ", Decided to jam this packet!");
    ...
    {
      NS_LOG_DEBUG("ReactiveJammer:At Node #" << GetId() << ", Decided NOT to jam this packet!");
    }

    if (m_reactToMitigation)
    {
      NS_LOG_DEBUG("ReactiveJammer:At Node #" << GetId() << ", React to mitigation enabled! Rescheduling RX Timeout");
    ...
    {
      NS_LOG_DEBUG("ReactiveJammer:At Node #" << GetId() << ", React to mitigation disabled!");
    }
  ...
  ReactiveJammer::DoEndRxHandler(Ptr<Packet> packet, double averageRss)
  {
    ...
    NS_LOG_DEBUG("ReactiveJammer:At Node #" << GetId() << ", Ignoring incoming packet!");
    return false;
  }
  ...
  ReactiveJammer::DoEndTxHandler(Ptr<Packet> packet, double txPower)
  {
    ...
    NS_LOG_DEBUG("ReactiveJammer:At Node #" << GetId() << ", Done sending jamming signal with power = " << txPower);
  }
  ...
  ReactiveJammer::IsPacketToBeJammed(Ptr<Packet> packet)
  {
    ...
    NS_LOG_DEBUG("ReactiveJammer:At Node #" << GetId() << ", Deciding whether to react to packet!");
    ...
    case ENERGY_AWARE:
      ...
      NS_LOG_DEBUG("ReactiveJammer:At Node #" << GetId() << ", Energy fraction = " << energyFraction);
      // make probabilistic decision based on energy fraction
      return (m_random->GetValue() < energyFraction);
    case FIXED_PROBABILITY:
      NS_LOG_DEBUG("ReactiveJammer:At Node #" << GetId() << ", Fixed probability " << m_fixedProbability);
      // make probabilistic decision based on fixed probability
      return (m_random->GetValue() < m_fixedProbability);
    default:
      NS_FATAL_ERROR("ReactiveJammer:At Node #" << GetId() << ", Error! Unknown strategy of reaction.");
      break;
    }
    return false;
  }
  ...
  {
    NS_LOG_FUNCTION(this);
    NS_LOG_DEBUG("ReactiveJammer:At Node #" << GetId() << ", Sending jamming signal with power = " << m_txPower << " W");

    // send jamming signal
    double actualPower = m_utility->SendJammingSignal(m_txPower, m_jammingDuration);
    if (actualPower != 0.0)
    {
      NS_LOG_DEBUG("ReactiveJammer:At Node #" << GetId() << ", Jamming signal sent with power = " << actualPower << " W");
    }
    else
    {
      NS_LOG_ERROR("ReactiveJammer:At Node #" << GetId() << ", Failed to send jamming signal!");
    }
  }
  ...
  {
    NS_LOG_FUNCTION(this);
    NS_LOG_DEBUG("ReactiveJammer:At Node #" << GetId() << ", RX timeout at " << Simulator::Now().GetSeconds() << "s");
    ...
    if (!m_reactToMitigation)
    {
      NS_LOG_DEBUG("ReactiveJammer:At Node #" << GetId() << ", React to mitigation is turned OFF!");
      return;
    }
    ...
    NS_LOG_DEBUG("ReactiveJammer:At Node #" << GetId() << ", Switching from channel " << currentChannel << " >-> " << nextChannel);
